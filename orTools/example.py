...
for i, courier in enumerate(couriers):
    ...
    # Курьер имеет конкретные типы бутылок, если capacity_12 и capacity_19 не равны нулю
    if capacity_12 > 0 or capacity_19 > 0:
...
if couriers_with_specific_capacity:
    ...
    for courier in couriers:
        ...
        # Если у курьера есть конкретные типы бутылок - используем их
        if capacity_12 > 0 or capacity_19 > 0:
...
for i, order in enumerate(orders):
    ...
    for courier_id in range(num_couriers):
        if courier_id in couriers_with_general_capacity:
            ...
            # Курьер с конкретными типами - проверяем совместимость
...
if solution:
    ...
    for vehicle_id in range(num_couriers):
        ...
        if route_orders:
            ...
            courier_has_specific_bottles = courier_capacity_12 > 0 or courier_capacity_19 > 0
            ...
            if courier_has_specific_bottles:
                ...
                if total_bottles_12 <= courier_capacity_12 and total_bottles_19 <= courier_capacity_19:
                    ...
                    if total_bottles_12 > courier_capacity_12:
                        print(f"     Не хватает 12л: {total_bottles_12 - courier_capacity_12}", file=sys.stderr)
                    if total_bottles_19 > courier_capacity_19:
                        print(f"     Не хватает 19л: {total_bottles_19 - courier_capacity_19}", file=sys.stderr)
            ...
            if courier_has_specific_bottles:
                ...
                courier_info["bottles_sufficient"] = (
                    total_bottles_12 <= courier_capacity_12 and 
                    total_bottles_19 <= courier_capacity_19
                )
                if not courier_info["bottles_sufficient"]:
                    courier_info["bottles_shortage"] = {
                        "bottles_12": max(0, total_bottles_12 - courier_capacity_12),
                        "bottles_19": max(0, total_bottles_19 - courier_capacity_19)
                    }
            else:
