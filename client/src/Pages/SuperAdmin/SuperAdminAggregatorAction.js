...
export default function SuperAdminAggregatorAction() {
    ...
    const getOrders = (date) => {
        ...
        api.post("/getAllOrderForToday", {date}, {
        ...
        api.post("/getActiveCourierAggregators", {date}, {
            headers: { "Content-Type": "application/json" },
        }).then((res) => {
        ...
        api.post("/getAllCouriersWithOrderCount", {date}, {
            headers: { "Content-Type": "application/json" },
        }).then((res) => {
    ...
    const handleAssignOrder = async (courierId) => {
        ...
        try {
            ...
            if (response.data.success) {
                // Обновляем данные
                const ordersRes = await api.post("/getAllOrderForToday", {date}, {
                    headers: { "Content-Type": "application/json" },
                });
                setOrders(ordersRes.data.orders);
                
                const couriersRes = await api.post("/getActiveCourierAggregators", {date}, {
                    headers: { "Content-Type": "application/json" },
                });
                setCouriers(couriersRes.data.couriers);
                
                const allCouriersRes = await api.post("/getAllCouriersWithOrderCount", {date}, {
                    headers: { "Content-Type": "application/json" },
                });
    ...
    return <div className="relative">
        <Container role={userData?.role}>
            ...
            <Li>
                ...
                <MyButton click={() => {getOrders(date)}}>Применить</MyButton>
            </Li>
            <Div />
                ...
                <div className="lg:hidden mt-5 bg-white bg-opacity-90 p-4 rounded-lg shadow-lg z-10 text-black">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex items-center">
                            ...
                            <span className="text-sm">Физ лица</span>
                        </div>
                        <div className="flex items-center">
                            <div className="w-4 h-4 bg-gray-400 mr-2"></div>
                            <span className="text-sm">Юр лица</span>
                        </div>
